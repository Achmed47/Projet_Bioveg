...
/*  $Id: svloader.js 36120 2016-08-12 21:14:19Z borodine $
 * ===========================================================================
 *
...
(function(){
    ...
    var externJS = (srcjs[2] != document.location.hostname ? '-extern.js' : '.js');
    var webNCBI = 'https://' + srcjs[2] + '/';
    var dotJS = (webNCBI.search(/www|qa/) !== 2 || document.location.href.indexOf('debug') > 0) ? '-debug.js' : '.js';
    var ExtJS6 = (document.location.href.indexOf('extjs6') > 0);
    ...
    var extPath = webNCBI + (ExtJS6 ? 'staff/lotovv/ext/ext-6.0.2/build/' : 'core/extjs/ext-5.0.1/build/');
//    var extPath = webNCBI + (ExtJS6 ? 'staff/borodine/ext/ext-6.0.1/build/' : 'core/extjs/ext-5.0.1/build/');
    ...
    var svResources = [
        {tag: 'link', attr: {rel: 'stylesheet', type: 'text/css', href: extPath
            + (ExtJS6 ? 'classic/theme-gray/resources/theme-gray-all.css' : 'packages/ext-theme-gray/build/resources/ext-theme-gray-all.css')}},
...
/*
    ...
    var onReady = function() {
        if (Ext.getVersion && Ext.getVersion().major >= 5) {
            ...
            try {
                initSeqView3();
            } catch(e) {alert('Current version of Sequence Viewer works only in ExtJS ver. 5.+ environment!');}
        }
        ...
        insertElement('jQuery', function(){
            insertElement('UUD', function(){ insertElement('TMS'); });
            insertElement('ncbi');
        });

        SeqView.extPath = extPath;
        SeqView.extImagesPath = extPath + (ExtJS6
            ? 'classic/theme-gray/resources/images/'
            : 'packages/ext-theme-gray/build/resources/images/');
    ...
    insertElement(extScripts[0], finalLoad);
...
/*  $Id: globals.js 36121 2016-08-12 21:33:19Z joukovv $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
Ext.define('Ext.sv.Window', {
    extend: 'Ext.Window',
    initComponent: function() {
        if (this.app && !this.renderTo && SeqView.geneIE) {
                this.renderTo = this.app.m_DivId;
        }
        this.callParent(arguments);
    },
    show: function() {
        var posX = window.pageXOffset + window.innerWidth/2;
        var posY = window.pageYOffset + window.innerHeight/2;
        if (!this.renderTo && this.width && this.height) {
            try {
                this.setPosition(posX - this.width/2, posY - this.height/2);
                posY = -1;
            } catch(e) {}
        }
        this.callParent(arguments);
        if (posY >= 0 && !this.renderTo) this.setPosition(posX - this.getWidth()/2, posY - this.getHeight()/2);
    }
});
...
//         this.body.setStyle('position', 'absolute');
    ...
    toggelPin: function(o, tool) {
        if (!this.pinned) {
            ...
            tool.setType('unpin');
            SeqView.ToolTip.superclass.onHide.call(this);
        }
        this.autoHide = !(this.pinned = !this.pinned);
    },
    update: function(html) {
        if (this.adjustWidth) delete this.width; // = 'auto';
        SeqView.ToolTip.superclass.update.call(this, html);
        this.updateLayout();
        this.doWidthAdjusting();
    }
});
...
SeqView.showAboutMessage = function() {
    ...
    + SeqView.getVersionString() + '<br><br><span style="font-size:10px;">Revision: 36129, install date: 2016-08-16 15:46</span><br>';
...
function setActiveScriptBase() {
  ...
  var base = "https://www.ncbi.nlm.nih.gov/projects/sviewer";
  var host = "https://www.ncbi.nlm.nih.gov/";
  var scriptUrl = "";
...
/*  $Id: locator.js 35370 2016-04-28 19:39:46Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
/*  $Id: selection.js 35863 2016-07-01 16:12:21Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
        ...
        if (this.track) {
            ...
            if (this.isPinnable())
               this.addTool([
                   ...
                   { type: 'up', callback: this.collapseHandler, hidden: true, scope: this },
                   { type: 'down', callback: this.collapseHandler, hidden: true, scope: this }]);
        }
//        this.keepArea = {x:this.gview.getXY()[0], y:this.gview.getXY()[1],width:this.gview.getScreenWidth(), height:this.gview.getScreenHeight()};
    ...
    collapseHandler: function(o, tool, e) {
        ...
        if (!this.collapsedTT) SeqView.pingClick('6-4');
    },

    trackTTHandler: function(o, tool, e) {
        var app = this.selection.m_View.m_App;
        this.hide();
        SeqView.pingClick(tool.stat);
        switch (tool.type) {
            case 'close':
            ...
            case 'restore':
                ...
                app._getLinkToThisPageURL('full', SeqView.TM.tracksArrayToString(tracks));
                break;
        }
    },
/*
    onShow: function() {
        this.callParent(arguments);
        if (!this.selection.area.tooltip) this.mask('Loading...', 'x-mask-loading');
...
//        this.adjustWidth = true;
        ...
        var tools = this.header.getTools();
        if (tools.up) tools.up.hide();
        if (tools.down) tools.down.hide();
        if (tt.extraLinks) {
            if (!this.collapsedTT) {
               if (!tt.basicLinks) content += '<br/><b>Links & Tools</b>';
               content += '<br/>' + tt.extraLinks;
               if (tools.up) tools.up.show();
            } else {
                if (tools.down) tools.down.show();
            }
        }
    ...
    showPrime: function(unr, params) {
        ...
        params = params || {url: this.m_View.m_App.m_CGIs.Graphic, context: this,
            data: {objinfo: 1, id: unr.seq_id, unalignedregion: true, from: unr.from, to: unr.to,
                alignedtail: Math.min(unr.length - (unr.to - unr.from), 500), reverse: (unr.orientation == 'reverse')}};
        ...
        SeqView.App.simpleAjaxRequest(params);
    }
};
...
Ext.define('SeqView.SelectedRangeToolTip', {
    ...
    initComponent: function() {
        ...
        this.title = 'Range: ' + adjustedRange1 + ' .. ' + adjustedRange2;
        this.callParent(arguments);
        var mHeight = 0;
        ...
        toolbar_items.push({
            ...
            handler: function() {
                this.hide();
                this.gview.m_App.addMarker(this.range);
                this.gview.removeRangeSelection_2(this.range);
        ...
        Ext.MessageBox.prompt('Modify Range', 'Enter New Range:', function(btn, text)
        {
            ...
            this.gview.m_App.handlePos(text, {
                ...
                success: function(pos_range, options) {
                    ...
                    this.selection.range[0] = this.range[0] = pos_range[0];
                    this.selection.range[1] = this.range[1] = pos_range[1];
                    this.selection.updateCoords();

                    var adjustedRange=[this.gview.m_App.posToLocal(this.range[0]),this.gview.m_App.posToLocal(this.range[1])];
                    this.setTitle('Range: ' + (!this.gview.m_Flip
                        ? (adjustedRange[0] + ' .. ' + adjustedRange[1])
                        : (adjustedRange[1] + ' .. ' + adjustedRange[0])));
                },
    ...
    updateToolTip: function(range,e) {
        var adjustedRange,
            org = this.gview.m_App.m_Origin;
        if (!this.gview.m_Flip) adjustedRange = [range[0] - org, range[1] - org];
        else adjustedRange = [-range[1] - org, -range[0] - org];
        this.setTitle('Range: ' + adjustedRange[0] + ' .. ' + adjustedRange[1]);
        this.range = range;
...
SeqView.RangeSelection.prototype = {
    ...
    onMouseUp: function(e) {
        ...
        if (this.m_ToolTip) {
            ...
            if (el) {
                this.m_ToolTip.show();
                this.m_ToolTip.updateToolTip(this.range,e);
            }
...
};/*  $Id: markers.js 35857 2016-06-30 20:53:29Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
// addMarkerByData:
    ...
    MarkerDlg: function(view, pos, oldMarker){
        ...
        setMarkerDlg = new Ext.sv.Window({
    ...
    updateInfo: function() {
        ...
        SeqView.App.simpleAjaxRequest({url: this.getMarkerInfoURL(), context: this,
...
/*  $Id: view.js 36100 2016-08-10 15:19:30Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
/*  $Id: panorama.js 35431 2016-05-06 19:08:10Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
/*  $Id: textview.js 35465 2016-05-10 18:50:43Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
SeqView.TextView.prototype = {
    ...
    displaySequenceTextDlg: function(from_seq, len_seq) {
        ...
        this.m_SeqDlg = new Ext.sv.Window({
...
/*  $Id: alignview.js 35303 2016-04-21 16:39:21Z rudnev $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
/*  $Id: graphview.js 36029 2016-07-29 15:55:03Z falkrb $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
//    getScreenWidth: function() { return this.m_App.getWidth() },
        ...
        if (this.m_App.m_Embedded && this.m_App.m_Embedded == 'minimal') {
            ...
            if( this.m_App.m_Toolbar["name"] == true ){
                ...
                if (seqInfo.assm_info) {
                    ...
                    menu_items.push({text: 'Assembly ' + seqInfo.assm_info.name, scope: this,
                        tooltip: wrn ? 'WARNING! ' + wrn : 'Assembly context information',
                        icon:  SeqView.extImagesPath + 'shared/icon-' + (wrn ? 'warning.gif' : 'info.gif'),
                ...
                var linksAdd = function() {
                    ...
                    Ext.each(this.scope.m_App.m_Config.SeqInfo.links, function() {
                        ...
                        menu.add({
                            text: this.label, tooltip: this.help,
                    ...
                    menu_items.push(
                        ...
                        {text:'Add new Panel on Range', iconCls:'xsv-new_view', tooltip:'Create New Graphical Panel on Selected Range',
                            scope:this, handler:function() {
                                ...
                                if (range) this.removeRangeSelection(true);
                                else
                ...
                menu_items.push(
                    ...
                    {text:'Download', iconCls:'xsv-download-static',
                     menu:new Ext.menu.Menu({items:[
                        ...
                        handler: function () { pingTB('2-2-7-9-5'); this.downloadAllMarkers("fasta"); }},
    ...
    processTrackSets: function(sets, stdID) {
        ...
        cmp.menu.add({text: !inMyNCBI ? 'Sign in to NCBI' : 'Manage collections',
                handler: function(){
                    SeqView.pingClick('2-2-8-2', inMyNCBI ? 'NCBI_sign_in' : 'collections_manage');
                    window.open(SeqView.NCBI.host_url + '/myncbi/collections');}});
        if (inMyNCBI)
            cmp.menu.add({text: 'Save current tracks', handler: function() { scope.saveTrackSet(); }});
    },
...
// refresh:
    ...
    removeRangeSelection_2: function(range) {
        ...
        for (x = 0; x < l; x++) {
            if (sel_set[x].range == range) { // this function removes only when "range" argument is a "range" object of one of m_RangeSelectionSet objects
                sel_set[x].remove();
                sel_set.splice(x, 1);
                break;
            }
    ...
    removeRangeSelection: function(permanent, range) {
        ...
        if (typeof(range) !== 'undefined') {
            this.popRangeElement(range);
            range.remove();
        } else {
            for (var x = 0; x < sel_set.length; x++) sel_set[x].remove();
            this.m_RangeSelectionSet = [];
        }
        this.m_App.fireEvent('selection_changed', this);
    },
            ...
            if (!data.job_status) {
                ...
                it.img.empty = (typeof it.areas == 'undefined');
                 ...
                 if (jst == 'submitted' || jst == 'running' || jst == 'pending') {
                     ...
                     Ext.defer(SeqView.App.simpleAjaxRequest, 2000, self, [cfg]);
                 }
            }
        };
...
//        params.delaytest = 5;
        ...
        var deferred = Ext.defer(this.showLoadingImage, 3*1000, this);
...
//             console.log('pageY:', e.pageY,', clientY:', e.clientY,', offsetY:', e.offsetY,', layerY:', e.layerY, ', y:', e.y)
    ...
    processTracksImages: function(){
        ...
        for (var j = 0; j < data.images.length; j++) {
            ...
            if (image.hairlines && data.images.length > 1) {
                ...
                this.m_featMarkers.hls[j] = {hairlines: arrHLs, top: this.m_Height, height: image.h};
            ...
            for (var i = 0; i < image.areas.length; i++) {
                ...
                if (area.type & (SeqView.AreaFlags.Track | SeqView.AreaFlags.Sequence)) {
                    ...
                    if (area.type & SeqView.AreaFlags.Sequence) {
                        ...
                        newEl1.insertHtml('afterBegin','<img data-qtitle="5 \'" data-qwidth="80" data-qtip="5 Prime end" src="'
                        ...
                        newEl2.insertHtml('afterBegin','<img data-qtitle="5 \'" data-qwidth="80" data-qtip="5 Prime end" src="'
            ...
            if (!tracks[i].display_name) continue;
                ...
                if (track.area) {
                    ...
                    newEl.on('mouseenter',function(e) {
                        this.removeFloatingSelection();
                        if (e.target.area) {
                            if (this.m_tnameDiv) this.m_tnameDiv.setStyle('opacity', 0.3);
            ...
            for (i = 0; i < data.areas.length; i++) {
                ...
                if (area.type & SeqView.AreaFlags.Comment) {
                    ...
                    if (area.type & SeqView.AreaFlags.DrawBackground) {
                        if (this.m_App.m_Config.Options.curr_color == 'Color')
                            styles += 'background-color:rgb(180,180,240);opacity:0.6;';
                        else
                            styles += 'background-color:rgb(195,195,195);opacity:0.6;';
                    }
                    el.appendChild(newEl.dom);
    ...
    blinkAreas: function(options) {
        ...
        function blink(bnum){
            ...
            if (bnum-- > 0) { Ext.defer(blink, 300, this, [bnum]); return; }
    ...
    applyResultsOnLoad: function(data, options){
        ...
        for (var i = chNodes.length - 1; i >= 0; i--) {
            ...
            lButt.onclick = rButt.onclick = function(e) {
                ...
                e.preventDefault();
                if (gview.tmp_gotoFeat) return;
                this.style.background = 'url('+ SeqView.extImagesPath + 'grid/loading.gif) 0px 0px';
        ...
        new_img_el.addCls('sv-drag');
        new_img_el.set({id: 'imgRuler' + this.m_DivId, draggable: false});
        new_img_el = the_div.insertFirst(new_img_el);
        new_img_el.move('right', this.m_ScrollPix);
        new_img_el.show();

        this.m_bottomRulerHeight = this.m_topRulerHeight = trx[0].imgT;
        if (this.m_bottomRulerHeight) {
            var wrapDiv = new Ext.Element(document.createElement('div'));
            wrapDiv.set({id: 'track_RulerB'});
            wrapDiv.applyStyles('position:absolute; top:' + this.m_Height + 'px; width:' + this.m_Width
                + 'px; height:' + this.m_bottomRulerHeight + 'px;left:' + this.m_ScrollPix + 'px;');
            var img = new Ext.Element(document.createElement('img'));
            img.set({id: new_img_el.id + 'B', draggable: false, src: this.m_FromCgi.images[0].img_url});
        ...
        for (var tIdx = trx.length - 1; tIdx >= 0; tIdx--) {
            var track = trx[tIdx];
            var tname = 'track_' + ((!track.display_name.length && track.signature == 'sequence') ? (tIdx + 'SqTrk') : tIdx);
    ...
    createTrackDiv: function(track, width, id) {
        ...
        if (id.search('SqTrk') > 0) {
            var tpl = new Ext.Template('<img data-qtitle="Hint" data-qtip="Use Drag\'n\'Drop to re-order the tracks" src='
        ...
        img.addCls(['sv-drag', 'sv-highlight', 'sv-dblclick']);
    ...
    updateTitle: function( options ){
        ...
        if( tbar ){
            ...
            if( tbtitle ){
                ...
                tiptitle += ': ' + this.m_App.m_Config.SeqInfo.title;
                tiptitle += '</b><br/><br/>';
                tiptitle += r_range_from.commify() + '&nbsp;-&nbsp;' + r_range_to.commify() + '<br/>';

                tiptitle += (range[1] - range[0] + 1).commify() + '&nbsp;';
                tiptitle += (this.m_App.m_ViewParams['acc_type']=='protein' ? 'residues' : 'bases') + '&nbsp;shown';
...
//console.log(page_xy[1], (areas ? areas[0].bounds : ''));
    ...
    onMouseDown: function(e) {
        ...
        if (e.target.id.indexOf('imgRuler') == 0) {
            // Add range selection
            e.stopPropagation();
    ...
    onMouseUp: function(e) {
        ...
        if (this.m_scrolled) {
            ...
            if (this.m_ReqFrom != this.m_VisFromSeq) SeqView.pingClick('2-0-2');
        }
        this.m_XY = null;
        if (e.button == 0) {
            document.onmousemove = this.m_DocMouseMove;
    ...
    onMouseMove: function(e) {
        ...
        if (this.m_InDragAction) {
            ...
            if (this.m_App.m_PermConfId || e.getTarget().id.indexOf('imgRuler') == 0) delta_y = 0;
            ...
            if (Ext.fly(e.getTarget()).hasCls('sv-highlight')) { // GView
                this.highlightElement(e.getXY());
            }
    ...
    showContextMenu: function(xy, callback) {
        ...
        if (this.m_App.m_Markers.length > 0) {
            download_submenu.add({
                text: 'FASTA (All Markers)', scope: this,
                handler: function () { this.downloadAllMarkers("fasta"); SeqView.pingClick('2-1-9-7'); }
            });
        }
...
//        if (document.body.clientHeight > win.height) win.renderTo = this.m_App.m_DivId;
    ...
    setPreferences: function() {
        var app = this.m_App;
        var optWindow = new Ext.sv.Window({
            title: 'Preferences',
            app: app,
            layout: 'form',
            modal: true,
...
//        if (document.body.clientHeight > optWindow.height) optWindow.renderTo = this.m_App.m_DivId;
    ...
    showAssmInfo: function() {
        ...
        var seqInfo = this.m_App.m_Config.SeqInfo;
        var wrn = seqInfo.warning_message;
        var msg = 'Name: <a href="' + SeqView.NCBI.host_url + '/assembly/' + seqInfo.assm_context
            + '/" target=\"_blank\">' + seqInfo.assm_info.name
            + '</a><br/>TaxID: <a href="' + SeqView.NCBI.host_url + '/Taxonomy/Browser/wwwtax.cgi?id=' +
...
/*  $Id: tm_panels.js 35794 2016-06-23 21:31:25Z borodine $
*/
        ...
        Ext.apply(this, {
            ...
            items:[{
                ...
                items:[
                    ...
                    {xtype:'displayfield', id: 'sv-uplmsg' + this.cfgPanel.sviewApp.m_DivId, value: '', width:'95%',
...
/*  $Id: tm_dialog.js 36100 2016-08-10 15:19:30Z borodine $
*/
...
SeqView.TM.Common = {
    ...
    createMainPanel: function(categories) {
        var removeButton = function() {
            ...
            var win = new Ext.sv.Window({title: 'Confirmation', modal: true, labelWidth: 1,
                app: this.sviewApp,
...
SeqView.TM.modifyTrackDetails = function(gview, track) {
    ...
    var tsWindow = new Ext.sv.Window({
    ...
    reConfigure: function(checkDirty) {
        if (this.sviewApp) {
            if (checkDirty && !this.uploadPanel.uploadButt.disabled) {
                this.uploadPanel.submitData(this.uploadPanel.getComponent('loaddatapanel').getForm(),
...
SeqView.TM.modifyLegendSettings = function(gview, track, idx) {
    ...
    var lgWindow = new Ext.sv.Window({
        title: track.display_name,
        app: app,
        modal: true,
        width:550,
...
/*  $Id: tm_grids.js 35514 2016-05-16 22:10:14Z borodine $
*/
...
//        this.viewConfig = { listeners: { render: this.createTooltip } };
    ...
    createTooltip: function(view) {
    	view.tip = Ext.create('SeqView.ToolTip', {
            target: view.el,
...
/*  $Id: sviewapp.js 36100 2016-08-10 15:19:30Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
SeqView.Config.prototype = {
    ...
    load: function(cfg) {
        ...
        if (this.m_App.m_Panel.tmpPanel)
            this.m_App.m_Panel.tmpPanel.mask('Loading...');
    ...
    getObjIdTrack: function(id) {
        ...
        Ext.each(this.TrackConfig,
            function(track, ix) {
                ...
                Ext.each(track.annots, function(annot, idx){
                    str += (idx == 0 ? ',annots:' : '|') + SeqView.escapeTrackName(annot);
                });
...
// are initialized in loadAccession
    initLinks: function() {
     ...
     if (this.m_hiddenOptions.search('use_jsonp') >= 0)    SeqView.jsonType = 'JSONP';
            ...
            if (Ext.isIE) {
                SeqView.App.simpleAjaxRequest({url: this.m_CGIs.Link + '?version=short',
...
//                html:'<span id="string_ruler_unit" style="font-size: 11px; font-weight:bold;"></span>',
                header: false
            };
...
//  returns whether track configuration changed
    ...
    loadAccession: function() {
        ...
        var cfg = {callback: {success: this.infoLoaded, failure: this.infoFailed, scope: this}};
    ...
    infoFailed: function(data, text, res){
        ...
        this.m_Panel.add({html:
            'An internal error has occurred that prevents Sequence Viewer from displaying.<br>'
            + 'Technical details (seqconfig error): ' + (msg || text)});
    },
...
//                if (document.body.clientHeight > dlg.height) dlg.renderTo = this.m_DivId;
    ...
    handleVariations: function(term, options) {
        ...
        var completeTask = function() {
            ...
            if (!found && !extSearch) { //searching in not displayed tracks
                ...
                Ext.each(app.m_Config.TrackConfig, function(trk, idx) {
                    ...
                    rqNum++;
                    SeqView.App.simpleAjaxRequest({url: app.m_CGIs.Graphic, data: params,
        ...
        app.maskGraphViews(view, 'Searching...', 'x-mask-loading');
        return [this.AjaxRequest({
            url: this.m_CGIs.FeatSearch,
            data: params,
            context: this,
            ...
            try {
                ...
                promise.fail(function() {
                    ...
                    Ext.Msg.show({title: 'User data loading error',
                        ...
                        fn: function(btn) {
                            switch (btn) {
                                case 'no': app.showFeedbackDlg(fbText, document.location.href); // "break" is missed to continue data loading
...
/*  $Id: sviewapp_more.js 35858 2016-06-30 20:57:40Z borodine $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
...
Ext.apply(SeqView.App.prototype, {
    ...
    _getLinkToThisPageURL: function (mode, tracks_key, callback) {
        ...
        if (this.m_SearchDlg && this.m_SearchDlg.isVisible()) {
            ...
            if (view) {
                ...
                if (searchTerm) the_link += '&search=' + searchTerm.replace(/&/g, '%26');
            }
        }
    ...
    showSearchParamsDlg: function (view) {
        ...
        dlg.add({xtype: 'panel', bodyStyle: {background: 'inherit'}, border: false,
            html: 'Enter sequence position or range (possible range formats are '
            + '10k-20k, -20--10, -10k:-5, 5 to 515, -1m..1m)<br/><br/>'
...
*/                    }
            ...
            this.m_SearchDlg = new Ext.sv.Window({
                app: this,
                layout: 'fit',
    ...
    showFeedbackDlg: function (fbText, fbURL) {
        ...
        var fbCallback = function (the_link_url) {
            ...
            var feedbackDlg = new Ext.sv.Window({
                app: this,
                ...
                buttons: [
                  { text: 'Send', handler: function () {
                      ...
                      if (form.isValid()) {
                          ...
                          cfg_data['feedback-os'] = SeqView.os;
                          SeqView.makeTinyURL(the_link_url, function (res) {
